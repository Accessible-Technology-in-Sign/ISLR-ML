...
from lstm import SimpleLSTM, DoubleLSTM, ComplexLSTM
...
def main():
    ...
    model.load_state_dict(torch.load(f"/saves/{args.name}.pth"))

    ai_edge_torch.convert(model.to(cpu).eval(), input_cpu).export(f"/models/{args.signs}-{args.name}-cpu.tflite")
